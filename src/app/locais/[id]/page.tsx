  ...
  return (
    <div>
      ...
      <img
        src={fotoMaisRecente}
        alt={local.nome}
        className="w-full rounded-2xl object-cover max-h-72 mb-4 bg-gray-100"
      />

      <h1 className="text-xl font-bold">{local.nome}</h1>
      <p className="text-gray-500 text-sm mb-1">{local.cidade}</p>
      {local.endereco && <p className="text-gray-400 text-xs mb-4">{local.endereco}</p>}

      {todasAvaliacoes.length === 0 ? (
        <p className="text-gray-500 text-sm mb-6">Ainda sem avaliações.</p>
      ) : (
        <>
          <div className="bg-white rounded-2xl border border-gray-200 p-4 mb-4">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm text-gray-500">Nota final</span>
              <span className="text-2xl font-bold text-brand-600">{notaFinal.toFixed(1)}</span>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-4 mb-6">
            <p className="text-sm text-gray-500 mb-3">Avaliação do local</p>
            <div className="grid grid-cols-3 gap-2 text-center text-sm">
              <div>
                <p className="text-xs text-gray-500 mb-0.5">Bem servido</p>
              ...
              <div>
                <p className="text-xs text-gray-500 mb-0.5">Limpeza</p>
          ...
          <li key={sessao.id} className="bg-white rounded-2xl border border-gray-200 p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium">
                ...
                {sessao.isHistorico && (
                  <span className="ml-2 text-xs text-gray-400">(histórico)</span>
                )}
              </span>
            ...
            <ul className="space-y-1">
              {sessao.avaliacoes.map((a) => (
                <li key={a.id} className="flex items-center justify-between text-sm text-gray-600">
